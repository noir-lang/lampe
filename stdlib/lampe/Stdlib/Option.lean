...
theorem expect_spec {p T N MT v message}
    (v_is_some : (toOption v).isSome) :
    STHoare p env ⟦⟧
      («std::option::Option::expect».call h![T, N, MT] h![v, message])
...
theorem unwrap_or_spec {p T v default} :
    STHoare p env ⟦⟧
      («std::option::Option::unwrap_or».call h![T] h![v, default])
...
theorem unwrap_or_else_none_spec {p T E v default default_b P Q}
    ...
    STHoare p env
      (P ⋆ [λdefault ↦ default_b])
      («std::option::Option::unwrap_or_else».call h![T, E] h![v, default])
      (fun r => Q r ⋆ [λdefault ↦ default_b]) := by
  enter_decl
  steps
  apply STHoare.ite_intro_of_false
...
theorem unwrap_unchecked_spec {p T v} :
    STHoare p env ⟦⟧
      («std::option::Option::unwrap_unchecked».call h![T] h![v])
