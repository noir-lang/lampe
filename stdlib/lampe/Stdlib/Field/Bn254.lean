...
abbrev PLO := «std::field::bn254::PLO»
abbrev PHI := «std::field::bn254::PHI»
abbrev TWO_POW_128 := «std::field::bn254::TWO_POW_128»
...
theorem lte_hint_intro {p a b} :
    STHoare p env ⟦⟧
      («std::field::bn254::lte_hint».call h![] h![a, b])
      (fun _ => ⟦⟧) := by
  enter_decl
  steps
...
theorem decompose_hint_intro {p x} :
    STHoare p env ⟦⟧
      («std::field::bn254::decompose_hint».call h![] h![x])
      (fun _ => ⟦⟧) := by
  enter_decl
  steps
...
theorem assert_gt_limbs_intro {p a b} [Prime.BitsGT p 129] :
    STHoare p env ⟦⟧
      («std::field::bn254::assert_gt_limbs».call h![] h![a, b])
      (fun _ => ⟦
...
theorem decompose_intro {p x} [Bn254.Prime p] :
    STHoare p env ⟦⟧
      («std::field::bn254::decompose».call h![] h![x])
      (fun r =>
        ∃∃ xlo xhi,
          r = (xlo, xhi, ()) ∧
          xlo.val < pow128 ∧
...
theorem assert_gt_intro {p a b} [Bn254.Prime p] :
    STHoare p env ⟦⟧
      («std::field::bn254::assert_gt».call h![] h![a, b])
...
theorem assert_lt_intro {p a b} [Bn254.Prime p] :
    STHoare p env ⟦⟧
      («std::field::bn254::assert_lt».call h![] h![a, b])
      (fun _ => a.val < b.val) := by
  enter_decl
...
theorem field_less_than_intro {p x y} :
    STHoare p env ⟦⟧
      («std::field::field_less_than».call h![] h![x, y])
      (fun _ => ⟦⟧) := by
  enter_decl
  steps
...
theorem gt_intro {p a b} [Bn254.Prime p] :
    STHoare p env ⟦⟧
      («std::field::bn254::gt».call h![] h![a, b])
...
theorem lt_intro {p a b} [Bn254.Prime p] :
    STHoare p env ⟦⟧
      («std::field::bn254::lt».call h![] h![a, b])
      (fun r => r = decide (a.val < b.val)) := by
  enter_decl
  steps [gt_intro (p := p)]
  rename_i r hret
