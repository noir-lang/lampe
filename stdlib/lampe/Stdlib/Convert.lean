...
@[reducible]
def «from» {p}
    (generics : HList Kind.denote «std::convert::From».«#genericKinds»)
    (Self : Tp)
    (associatedTypes : HList Kind.denote «std::convert::From».«#associatedTypesKinds»)
    (fnGenerics : HList Kind.denote «std::convert::From».«from».«#genericKinds»)
  : HList (Tp.denote p)
      («std::convert::From».«from».«#inputs» generics Self associatedTypes fnGenerics)
  → Expr (Tp.denote p)
      («std::convert::From».«from».«#output» generics Self associatedTypes fnGenerics) :=
  «std::convert::From».«from» generics Self associatedTypes fnGenerics
...
def hasFromImpl (env : Env) (source : Tp) (self : Tp) :=
  «std::convert::From».hasImpl env h![source] self

/-- A shorthand for a call to the `std::convert::Into::into` method. -/
@[reducible]
def into {p}
    (generics : HList Kind.denote «std::convert::Into».«#genericKinds»)
    (Self : Tp)
    (associatedTypes : HList Kind.denote «std::convert::Into».«#associatedTypesKinds»)
    (fnGenerics : HList Kind.denote «std::convert::Into».into.«#genericKinds»)
  : HList (Tp.denote p)
      («std::convert::Into».into.«#inputs» generics Self associatedTypes fnGenerics)
  → Expr (Tp.denote p)
      («std::convert::Into».into.«#output» generics Self associatedTypes fnGenerics) :=
  «std::convert::Into».into generics Self associatedTypes fnGenerics
...
def hasIntoImpl (env : Env) (target : Tp) (self : Tp) :=
  «std::convert::Into».hasImpl env h![target] self

/-- A shorthand for a call to the `std::convert::AsPrimitive::as_` method. -/
@[reducible]
def as_ {p}
    (generics : HList Kind.denote «std::convert::AsPrimitive».«#genericKinds»)
    (Self : Tp)
    (associatedTypes : HList Kind.denote «std::convert::AsPrimitive».«#associatedTypesKinds»)
    (fnGenerics : HList Kind.denote «std::convert::AsPrimitive».as_.«#genericKinds»)
  : HList (Tp.denote p)
      («std::convert::AsPrimitive».as_.«#inputs» generics Self associatedTypes fnGenerics)
  → Expr (Tp.denote p)
      («std::convert::AsPrimitive».as_.«#output» generics Self associatedTypes fnGenerics) :=
  «std::convert::AsPrimitive».as_ generics Self associatedTypes fnGenerics
...
def hasAsPrimitiveImpl (env : Env) (target : Tp) (self : Tp) :=
  «std::convert::AsPrimitive».hasImpl env h![target] self
