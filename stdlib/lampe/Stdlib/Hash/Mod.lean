...
theorem blake3_spec {p} {N : U 32}
    {input : Tp.denote p ((Tp.u 8).array N)} :
    STHoare p env ⟦⟧
      («std::hash::blake3».call h![N] h![input])
      (fun r => r = Lampe.Crypto.Blake3.blake3Hash input) := by
  enter_decl
...
theorem field_hash_spec {p H stateRef}
    ...
    (h_write_spec : STHoare p env
      [stateRef ↦ ⟨H, state⟩]
      («std::hash::Hasher».write h![] H h![] h![] h![stateRef, self])
      (fun _ => [stateRef ↦ ⟨H, final⟩]))
    : STHoare p env
        [stateRef ↦ ⟨H, state⟩]
        («std::hash::Hash».hash h![] .field h![] h![H] h![self, stateRef])
...
theorem u1_hash_spec {p H stateRef}
    ...
    : STHoare p env
        [stateRef ↦ ⟨H, state⟩]
        («std::hash::Hash».hash h![] (.u 1) h![] h![H] h![self, stateRef])
        (fun _ => [stateRef ↦ ⟨H, final⟩]) := by
  resolve_trait
  steps [h_write_spec]
...
theorem u8_hash_spec {p H stateRef}
    ...
    : STHoare p env
        [stateRef ↦ ⟨H, state⟩]
        («std::hash::Hash».hash h![] (.u 8) h![] h![H] h![self, stateRef])
        (fun _ => [stateRef ↦ ⟨H, final⟩]) := by
  resolve_trait
  steps [h_write_spec]
...
theorem u16_hash_spec {p H stateRef}
    ...
    : STHoare p env
        [stateRef ↦ ⟨H, state⟩]
        («std::hash::Hash».hash h![] (.u 16) h![] h![H] h![self, stateRef])
        (fun _ => [stateRef ↦ ⟨H, final⟩]) := by
  resolve_trait
  steps [h_write_spec]
...
theorem u32_hash_spec {p H stateRef}
    ...
    : STHoare p env
        [stateRef ↦ ⟨H, state⟩]
        («std::hash::Hash».hash h![] (.u 32) h![] h![H] h![self, stateRef])
        (fun _ => [stateRef ↦ ⟨H, final⟩]) := by
  resolve_trait
  steps [h_write_spec]
...
theorem u64_hash_spec {p H stateRef}
    ...
    : STHoare p env
        [stateRef ↦ ⟨H, state⟩]
        («std::hash::Hash».hash h![] (.u 64) h![] h![H] h![self, stateRef])
        (fun _ => [stateRef ↦ ⟨H, final⟩]) := by
  resolve_trait
  steps [h_write_spec]
...
theorem u128_hash_spec {p H stateRef}
    ...
    : STHoare p env
        [stateRef ↦ ⟨H, state⟩]
        («std::hash::Hash».hash h![] (.u 128) h![] h![H] h![self, stateRef])
        (fun _ => [stateRef ↦ ⟨H, final⟩]) := by
  resolve_trait
  steps [h_write_spec]
...
theorem i8_hash_spec {p H stateRef}
    ...
    : STHoare p env
        [stateRef ↦ ⟨H, state⟩]
        («std::hash::Hash».hash h![] (.i 8) h![] h![H] h![self, stateRef])
        (fun _ => [stateRef ↦ ⟨H, final⟩]) := by
  resolve_trait
  steps [h_write_spec]
...
theorem i16_hash_spec {p H stateRef}
    ...
    : STHoare p env
        [stateRef ↦ ⟨H, state⟩]
        («std::hash::Hash».hash h![] (.i 16) h![] h![H] h![self, stateRef])
        (fun _ => [stateRef ↦ ⟨H, final⟩]) := by
  resolve_trait
  steps [h_write_spec]
...
theorem i32_hash_spec {p H stateRef}
    ...
    : STHoare p env
        [stateRef ↦ ⟨H, state⟩]
        («std::hash::Hash».hash h![] (.i 32) h![] h![H] h![self, stateRef])
        (fun _ => [stateRef ↦ ⟨H, final⟩]) := by
  resolve_trait
  steps [h_write_spec]
...
theorem i64_hash_spec {p H stateRef}
    ...
    : STHoare p env
        [stateRef ↦ ⟨H, state⟩]
        («std::hash::Hash».hash h![] (.i 64) h![] h![H] h![self, stateRef])
        (fun _ => [stateRef ↦ ⟨H, final⟩]) := by
  resolve_trait
  steps [h_write_spec]
...
theorem bool_hash_spec {p H stateRef}
    ...
    : STHoare p env
        [stateRef ↦ ⟨H, state⟩]
        («std::hash::Hash».hash h![] .bool h![] h![H] h![self, stateRef])
        (fun _ => [stateRef ↦ ⟨H, final⟩]) := by
  resolve_trait
  steps [h_write_spec]
...
theorem unit_hash_spec {p H stateRef}
    ...
    : STHoare p env
        [stateRef ↦ ⟨H, state⟩]
        («std::hash::Hash».hash h![] .unit h![] h![H] h![(), stateRef])
