...
theorem derive_generators_spec {p} {N M : U 32}
    ...
    {startIdx : U 32} :
    STHoare p env ⟦⟧
      («std::hash::derive_generators».call h![N, M]
...
theorem from_field_unsafe_spec {p} [Lampe.Crypto.Bn254.Prime p]
    {scalar : Fp p} :
    STHoare p env ⟦⟧
      («std::hash::from_field_unsafe».call h![] h![scalar])
      (fun r =>
        ∃∃ xlo xhi,
          r = Scalar.mk xlo xhi ∧
...
private theorem pedersen_commitment_with_separator_spec {p N}
    ...
    {separator : U 32} :
    STHoare p env ⟦⟧
      («std::hash::pedersen_commitment_with_separator».call h![N]
        h![input, separator])
      (fun r =>
...
private theorem pedersen_hash_with_separator_spec {p N}
    ...
    {separator : U 32} :
    STHoare p env ⟦⟧
      («std::hash::pedersen_hash_with_separator».call h![N]
        h![input, separator])
      (fun r =>
        ∃∃ Ss : List.Vector (Scalar.denote p) N.toNat,
          r = pointX
  ...
  set lenScalar : Tp.denote p Scalar.type :=
    HList.toTuple p h![(Builtin.CastTp.cast N : Fp p), (Builtin.CastTp.cast ↑(0 : Fp p) : Fp p)]
      (some «std::embedded_curve_ops::EmbeddedCurveScalar».name) with hLenScalar_def
...
theorem pedersen_commitment_spec_canonical {p N}
    ...
    {input : Tp.denote p (Tp.field.array N)} :
    STHoare p env ⟦⟧
      («std::hash::pedersen_commitment».call h![N] h![input])
      (fun r =>
...
theorem pedersen_hash_spec_canonical {p N}
    ...
    {input : Tp.denote p (Tp.field.array N)} :
    STHoare p env ⟦⟧
      («std::hash::pedersen_hash».call h![N] h![input])
      (fun r =>
