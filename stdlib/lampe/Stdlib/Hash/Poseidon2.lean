...
theorem default_spec {p}
    : STHoare p env ⟦⟧
        (Lampe.Stdlib.Default.default h![]
          («std::hash::poseidon2::Poseidon2Hasher».tp h![]) h![] h![] h![])
...
theorem rate_spec {p}
    : STHoare p env ⟦⟧
        («std::hash::poseidon2::RATE».call h![] h![])
        (fun r => r = (3 : U 32)) := by
  enter_decl
  steps
  rename_i hrate
...
theorem new_spec {p} {iv : Tp.denote p .field}
    : STHoare p env ⟦⟧
        («std::hash::poseidon2::Poseidon2::new».call h![] h![iv])
...
theorem perform_duplex_spec {p selfRef}
    ...
    : STHoare p env
        ...
        («std::hash::poseidon2::Poseidon2::perform_duplex».call h![] h![selfRef])
        (fun _ =>
          [selfRef ↦ ⟨Poseidon2Tp,
...
theorem absorb_spec {p selfRef}
    ...
    : STHoare p env
        ...
        («std::hash::poseidon2::Poseidon2::absorb».call h![] h![selfRef, input])
...
theorem squeeze_spec {p selfRef}
    ...
    : STHoare p env
        [selfRef ↦ ⟨Poseidon2Tp, self⟩]
        («std::hash::poseidon2::Poseidon2::squeeze».call h![] h![selfRef])
...
theorem hash_internal_spec {p}
    ...
    : STHoare p env ⟦⟧
        («std::hash::poseidon2::Poseidon2::hash_internal».call h![N]
...
theorem hash_spec {p}
    ...
    : STHoare p env ⟦⟧
        («std::hash::poseidon2::Poseidon2::hash».call h![N]
          h![input, messageSize])
        (fun r => r = out) := by
  enter_decl
...
theorem hasher_write_spec {p selfRef}
    ...
    : STHoare p env
        ...
        («std::hash::Hasher».write h![] Poseidon2HasherTp h![] h![]
          h![selfRef, input])
...
theorem hasher_finish_spec {p}
    ...
    (hout : Crypto.Poseidon2.Sponge.hash?
        ...
        false = some out)
    : STHoare p env ⟦⟧
        («std::hash::Hasher».finish h![] Poseidon2HasherTp h![] h![]
