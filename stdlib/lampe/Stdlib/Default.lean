...
@[reducible]
def default {p}
    (generics : HList Kind.denote «std::default::Default».«#genericKinds»)
    (Self : Tp)
    (associatedTypes : HList Kind.denote «std::default::Default».«#associatedTypesKinds»)
    (fnGenerics : HList Kind.denote «std::default::Default».default.«#genericKinds»)
  : HList (Tp.denote p)
      («std::default::Default».default.«#inputs» generics Self associatedTypes fnGenerics)
  → Expr (Tp.denote p)
      («std::default::Default».default.«#output» generics Self associatedTypes fnGenerics) :=
  «std::default::Default».default generics Self associatedTypes fnGenerics

/--
Asserts that the provided `tp` has an implementation of `std::default::Default` in the environment.
-/
@[reducible]
def hasDefaultImpl (env : Env) (tp : Tp) := «std::default::Default».hasImpl env h![] tp
