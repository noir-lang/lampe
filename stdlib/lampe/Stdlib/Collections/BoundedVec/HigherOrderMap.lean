      ...
      (Expr.letIn
        (Expr.loop (↑0) MaxLen fun i =>
          expr!![
            {
              let lenFn =
                («std::collections::bounded_vec::BoundedVec::len»<T, MaxLen : u32>
                  as λ(splice!(bvTp T MaxLen)) -> u32);
              let selfLen = (lenFn as λ(splice!(bvTp T MaxLen)) -> u32)(self);
              let cond = (#_uLt returning bool)(i, selfLen);
              if cond then {
                let getUncheckedFn =
                  («std::collections::bounded_vec::BoundedVec::get_unchecked»<T, MaxLen : u32>
...
theorem map_effectful_spec {p T MaxLen Out Env self f fb}
  ...
  : STHoare p env (inv [] [] ⋆ [λf ↦ fb])
      («std::collections::bounded_vec::BoundedVec::map».call h![T, MaxLen, Out, Env]
        h![self, f])
...
theorem mapi_effectful_spec {p T MaxLen Out Env self f fb}
  ...
  : STHoare p env (inv [] [] ⋆ [λf ↦ fb])
      («std::collections::bounded_vec::BoundedVec::mapi».call h![T, MaxLen, Out, Env]
        h![self, f])
      (fun r => ⟦wellFormed r⟧ ⋆ inv (embed self) (embed r)) := by
  enter_decl
  have hb : bounded self := (bounded_iff_wellFormed (v := self)).2 hwf_self
...
theorem any_spec {p T MaxLen Env self f fb}
  ...
  : STHoare p env
      (inv [] false ⋆ [λf ↦ fb])
      («std::collections::bounded_vec::BoundedVec::any».call h![T, MaxLen, Env]
        h![self, f])
...
theorem for_each_spec {T Env p MaxLen self f fb}
  ...
  : STHoare p env
      (Inv [] ⋆ [λf ↦ fb])
      («std::collections::bounded_vec::BoundedVec::for_each».call h![T, MaxLen, Env]
        h![self, f])
      (fun _ => Inv (embed self) ⋆ [λf ↦ fb]) := by
  enter_decl
  have hb : bounded self := (bounded_iff_wellFormed (v := self)).2 hwf_self
...
theorem for_eachi_spec {T Env p MaxLen self f fb}
  ...
  : STHoare p env
      (inv [] ⋆ [λf ↦ fb])
      («std::collections::bounded_vec::BoundedVec::for_eachi».call h![T, MaxLen, Env]
        h![self, f])
      (fun _ => inv (embed self) ⋆ [λf ↦ fb]) := by
  enter_decl
  have hb : bounded self := (bounded_iff_wellFormed (v := self)).2 hwf_self
