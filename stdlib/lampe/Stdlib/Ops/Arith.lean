...
@[reducible]
def add {p}
    (generics : HList Kind.denote «std::ops::arith::Add».«#genericKinds»)
    (Self : Tp)
    (associatedTypes : HList Kind.denote «std::ops::arith::Add».«#associatedTypesKinds»)
    (fnGenerics : HList Kind.denote «std::ops::arith::Add».add.«#genericKinds»)
  : HList (Tp.denote p)
      («std::ops::arith::Add».add.«#inputs» generics Self associatedTypes fnGenerics)
  → Expr (Tp.denote p)
      («std::ops::arith::Add».add.«#output» generics Self associatedTypes fnGenerics) :=
  «std::ops::arith::Add».add generics Self associatedTypes fnGenerics
...
@[reducible]
def sub {p}
    (generics : HList Kind.denote «std::ops::arith::Sub».«#genericKinds»)
    (Self : Tp)
    (associatedTypes : HList Kind.denote «std::ops::arith::Sub».«#associatedTypesKinds»)
    (fnGenerics : HList Kind.denote «std::ops::arith::Sub».sub.«#genericKinds»)
  : HList (Tp.denote p)
      («std::ops::arith::Sub».sub.«#inputs» generics Self associatedTypes fnGenerics)
  → Expr (Tp.denote p)
      («std::ops::arith::Sub».sub.«#output» generics Self associatedTypes fnGenerics) :=
  «std::ops::arith::Sub».sub generics Self associatedTypes fnGenerics
...
@[reducible]
def mul {p}
    (generics : HList Kind.denote «std::ops::arith::Mul».«#genericKinds»)
    (Self : Tp)
    (associatedTypes : HList Kind.denote «std::ops::arith::Mul».«#associatedTypesKinds»)
    (fnGenerics : HList Kind.denote «std::ops::arith::Mul».mul.«#genericKinds»)
  : HList (Tp.denote p)
      («std::ops::arith::Mul».mul.«#inputs» generics Self associatedTypes fnGenerics)
  → Expr (Tp.denote p)
      («std::ops::arith::Mul».mul.«#output» generics Self associatedTypes fnGenerics) :=
  «std::ops::arith::Mul».mul generics Self associatedTypes fnGenerics
...
@[reducible]
def div {p}
    (generics : HList Kind.denote «std::ops::arith::Div».«#genericKinds»)
    (Self : Tp)
    (associatedTypes : HList Kind.denote «std::ops::arith::Div».«#associatedTypesKinds»)
    (fnGenerics : HList Kind.denote «std::ops::arith::Div».div.«#genericKinds»)
  : HList (Tp.denote p)
      («std::ops::arith::Div».div.«#inputs» generics Self associatedTypes fnGenerics)
  → Expr (Tp.denote p)
      («std::ops::arith::Div».div.«#output» generics Self associatedTypes fnGenerics) :=
  «std::ops::arith::Div».div generics Self associatedTypes fnGenerics
...
@[reducible]
def rem {p}
    (generics : HList Kind.denote «std::ops::arith::Rem».«#genericKinds»)
    (Self : Tp)
    (associatedTypes : HList Kind.denote «std::ops::arith::Rem».«#associatedTypesKinds»)
    (fnGenerics : HList Kind.denote «std::ops::arith::Rem».rem.«#genericKinds»)
  : HList (Tp.denote p)
      («std::ops::arith::Rem».rem.«#inputs» generics Self associatedTypes fnGenerics)
  → Expr (Tp.denote p)
      («std::ops::arith::Rem».rem.«#output» generics Self associatedTypes fnGenerics) :=
  «std::ops::arith::Rem».rem generics Self associatedTypes fnGenerics
...
@[reducible]
def neg {p}
    (generics : HList Kind.denote «std::ops::arith::Neg».«#genericKinds»)
    (Self : Tp)
    (associatedTypes : HList Kind.denote «std::ops::arith::Neg».«#associatedTypesKinds»)
    (fnGenerics : HList Kind.denote «std::ops::arith::Neg».neg.«#genericKinds»)
  : HList (Tp.denote p)
      («std::ops::arith::Neg».neg.«#inputs» generics Self associatedTypes fnGenerics)
  → Expr (Tp.denote p)
      («std::ops::arith::Neg».neg.«#output» generics Self associatedTypes fnGenerics) :=
  «std::ops::arith::Neg».neg generics Self associatedTypes fnGenerics
...
@[reducible]
def wrapping_add {p}
    (generics : HList Kind.denote «std::ops::arith::WrappingAdd».«#genericKinds»)
    (Self : Tp)
    (associatedTypes : HList Kind.denote «std::ops::arith::WrappingAdd».«#associatedTypesKinds»)
    (fnGenerics : HList Kind.denote «std::ops::arith::WrappingAdd».wrapping_add.«#genericKinds»)
  : HList (Tp.denote p)
      («std::ops::arith::WrappingAdd».wrapping_add.«#inputs» generics Self associatedTypes fnGenerics)
  → Expr (Tp.denote p)
      («std::ops::arith::WrappingAdd».wrapping_add.«#output» generics Self associatedTypes fnGenerics) :=
  «std::ops::arith::WrappingAdd».wrapping_add generics Self associatedTypes fnGenerics
...
@[reducible]
def wrapping_sub {p}
    (generics : HList Kind.denote «std::ops::arith::WrappingSub».«#genericKinds»)
    (Self : Tp)
    (associatedTypes : HList Kind.denote «std::ops::arith::WrappingSub».«#associatedTypesKinds»)
    (fnGenerics : HList Kind.denote «std::ops::arith::WrappingSub».wrapping_sub.«#genericKinds»)
  : HList (Tp.denote p)
      («std::ops::arith::WrappingSub».wrapping_sub.«#inputs» generics Self associatedTypes fnGenerics)
  → Expr (Tp.denote p)
      («std::ops::arith::WrappingSub».wrapping_sub.«#output» generics Self associatedTypes fnGenerics) :=
  «std::ops::arith::WrappingSub».wrapping_sub generics Self associatedTypes fnGenerics
...
@[reducible]
def wrapping_mul {p}
    (generics : HList Kind.denote «std::ops::arith::WrappingMul».«#genericKinds»)
    (Self : Tp)
    (associatedTypes : HList Kind.denote «std::ops::arith::WrappingMul».«#associatedTypesKinds»)
    (fnGenerics : HList Kind.denote «std::ops::arith::WrappingMul».wrapping_mul.«#genericKinds»)
  : HList (Tp.denote p)
      («std::ops::arith::WrappingMul».wrapping_mul.«#inputs» generics Self associatedTypes fnGenerics)
  → Expr (Tp.denote p)
      («std::ops::arith::WrappingMul».wrapping_mul.«#output» generics Self associatedTypes fnGenerics) :=
  «std::ops::arith::WrappingMul».wrapping_mul generics Self associatedTypes fnGenerics
...
theorem two_pow_64_spec {p}
  : STHoare p env ⟦⟧
    («std::ops::arith::two_pow_64».call h![] h![])
...
theorem split_into_64_bit_limbs_spec {p a} [gt : Prime.BitsGT p 128]
  : STHoare p env ⟦⟧
    («std::ops::arith::split_into_64_bit_limbs».call h![] h![a])
