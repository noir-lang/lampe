...
theorem map_spec {p T N U Env l f fb}
  ...
  : STHoare p env
    ((inv [] []) ⋆ [λf ↦ fb])
    («std::array::map».call h![T, N, U, Env] h![l, f])
...
theorem mapi_spec {p T N U Env l f fb}
  ...
  : STHoare p env
    (inv [] [] ⋆ [λf ↦ fb])
    («std::array::mapi».call h![T, N, U, Env] h![l, f])
    (fun r => inv l.toList r.toList ⋆ [λf ↦ fb]) := by
  enter_decl
  steps
...
theorem for_each_spec {p T N Env l f fb}
  ...
  : STHoare p env
    (inv [] ⋆ [λf ↦ fb])
    («std::array::for_each».call h![T, N, Env] h![l, f])
...
theorem for_eachi_spec {p T N Env l f fb}
  ...
  : STHoare p env
    (inv [] ⋆ [λf ↦ fb])
    («std::array::for_eachi».call h![T, N, Env] h![l, f])
    (fun _ => inv l.toList ⋆ [λf ↦ fb]) := by
  enter_decl
  steps
  loop_inv nat fun i _ _ => inv (l.take i).toList ⋆ [λf ↦ fb]
...
theorem fold_spec {p T N U Env l a f fb}
  ...
  : STHoare p env
    (inv [] a ⋆ [λf ↦ fb])
    («std::array::fold».call h![T, N, U, Env] h![l, a, f])
...
theorem reduce_spec {p T N Env l f fb}
  ...
  : STHoare p env
    (inv [] l[0] ⋆ [λf ↦ fb])
    («std::array::reduce».call h![T, N, Env] h![l, f])
...
theorem all_spec {p T N Env l f fb}
  ...
  : STHoare p env
    (inv [] true ⋆ [λf ↦ fb])
    («std::array::all».call h![T, N, Env] h![l, f])
    (fun r => inv l.toList r ⋆ [λf ↦ fb]) := by
  enter_decl
  steps
...
theorem any_spec {p T N Env l f fb}
  ...
  : STHoare p env
    (inv [] false ⋆ [λf ↦ fb])
    («std::array::any».call h![T, N, Env] h![l, f])
    (fun r => inv l.toList r ⋆ [λf ↦ fb]) := by
  enter_decl
  steps
...
theorem sort_via_spec {p T N Env l f fb}
  ...
  : STHoare p env [λf ↦ fb]
    («std::array::sort_via».call h![T, N, Env] h![l, f])
