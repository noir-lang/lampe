...
@[reducible]
def type := «std::embedded_curve_ops::EmbeddedCurvePoint».tp h![]
...
@[reducible]
def type := «std::embedded_curve_ops::EmbeddedCurveScalar».tp h![]
...
theorem point_at_infinity_spec {p} :
    STHoare p env ⟦⟧
      («std::embedded_curve_ops::EmbeddedCurvePoint::point_at_infinity».call
        h![] h![])
...
theorem generator_spec {p} :
    STHoare p env ⟦⟧
      («std::embedded_curve_ops::EmbeddedCurvePoint::generator».call h![] h![])
...
theorem scalar_new_spec {p} {lo hi : Fp p} :
    STHoare p env ⟦⟧
      («std::embedded_curve_ops::EmbeddedCurveScalar::new».call
        h![] h![lo, hi])
...
private theorem point_neg_concrete_spec {p} {self : Point.denote p} :
    STHoare p env ⟦⟧
      («std::ops::arith::Neg».neg h![] Point.type h![] h![] h![self])
...
private theorem point_eq_concrete_spec {p} {self other : Point.denote p} :
    STHoare p env ⟦⟧
      («std::cmp::Eq».eq h![] Point.type h![] h![] h![self, other])
...
private theorem scalar_eq_concrete_spec {p} {self other : Scalar.denote p} :
    STHoare p env ⟦⟧
      («std::cmp::Eq».eq h![] Scalar.type h![] h![] h![self, other])
...
theorem embedded_curve_add_inner_spec {p}
    ...
    (hOnCurve :
      (curvePoint? point1).isSome ∧
        (curvePoint? point2).isSome) :
    STHoare p env ⟦⟧
      («std::embedded_curve_ops::embedded_curve_add_inner».call
        h![] h![point1, point2])
      (fun r =>
        r = encodeCurvePoint
          ((curvePoint? point1).get hOnCurve.1 +
...
theorem embedded_curve_add_spec {p}
    ...
    (hOnCurve :
      (curvePoint? point1).isSome ∧
        (curvePoint? point2).isSome) :
    STHoare p env ⟦⟧
      («std::embedded_curve_ops::embedded_curve_add».call
        h![] h![point1, point2])
      (fun r =>
        r = encodeCurvePoint
          ((curvePoint? point1).get hOnCurve.1 +
...
private theorem point_add_concrete_spec {p} {self other : Point.denote p}
    ...
    STHoare p env ⟦⟧
      («std::ops::arith::Add».add h![] Point.type h![] h![] h![self, other])
      (fun r =>
        r = encodeCurvePoint
...
private theorem point_double_concrete_spec {p} {self : Point.denote p}
    ...
    STHoare p env ⟦⟧
      («std::embedded_curve_ops::EmbeddedCurvePoint::double».call h![] h![self])
      (fun r =>
        r = encodeCurvePoint
...
private theorem point_sub_concrete_spec {p} {self other : Point.denote p}
    ...
    STHoare p env ⟦⟧
      («std::ops::arith::Sub».sub h![] Point.type h![] h![] h![self, other])
      (fun r =>
        r = encodeCurvePoint
          ((curvePoint? self).get hOnCurve.1 +
...
theorem scalar_from_field_spec {p} [Lampe.Crypto.Bn254.Prime p]
    {scalar : Fp p} :
    STHoare p env ⟦⟧
      («std::embedded_curve_ops::EmbeddedCurveScalar::from_field».call
        h![] h![scalar])
      (fun r =>
        ∃∃ lo hi,
          r = Scalar.mk lo hi ∧
...
theorem scalar_from_bytes_spec {p bytes offset}
    ...
    STHoare p env ⟦⟧
      («std::embedded_curve_ops::EmbeddedCurveScalar::from_bytes».call
        h![] h![bytes, offset])
      (fun r =>
        r =
          Scalar.mk
...
theorem scalar_hash_spec {p H stateRef}
    ...
    : STHoare p env
      [stateRef ↦ ⟨H, state⟩]
      («std::hash::Hash».hash h![] Scalar.type h![] h![H] h![self, stateRef])
      (fun _ => [stateRef ↦ ⟨H, final⟩]) := by
  resolve_trait
  reduce_fn_body
...
private theorem multi_scalar_mul_concrete_spec {p N}
    ...
    (hOnCurve : ∀ i, (curvePoint? (points.get i)).isSome) :
    STHoare p env ⟦⟧
      («std::embedded_curve_ops::multi_scalar_mul».call
...
private theorem fixed_base_scalar_mul_concrete_spec {p}
    ...
    STHoare p env ⟦⟧
      («std::embedded_curve_ops::fixed_base_scalar_mul».call h![] h![scalar])
      (fun r =>
        r = encodeCurvePoint
